...
/**
 * Every answer a profile is made of, apart from the contact details.
 *
 * Split out because the two endpoints that write a profile disagree about
 * exactly one thing - which contact details they will accept - and agree about
 * everything else. Setup takes whichever of email/phone the account is missing;
 * the edit screen can only ever move the phone, because the address the account
 * was verified with is what the account is. Both extend this.
 *
 * Which account is being written is in neither: it comes from the token the
 * guard verified. A body field would let any signed-in caller write to someone
 * else's profile by changing one value, and forbidNonWhitelisted rejects the
 * field outright if an old client sends it.
 */
